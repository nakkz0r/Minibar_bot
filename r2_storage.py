...
class R2Storage:
    # Имя объекта базы данных в бакете — всегда одно и то же,
    # чтобы при рестарте бот скачивал актуальную версию
    DB_OBJECT = "minibar.db"
    ...
    def __init__(self):
        ...
        if self.enabled:
            ...
            self.client = boto3.client(
                ...
                region_name="auto",
                # Таймауты и повторы: зависший сетевой вызов не подвесит бэкап навсегда
                config=Config(
                    connect_timeout=10,
                    read_timeout=60,
    ...
    async def download_db(self, db_path: str = "minibar.db") -> bool:
        """Скачивает базу из R2 в файл db_path."""
        return await asyncio.to_thread(self._download_file, self.DB_OBJECT, db_path)

    async def upload_db(self, db_path: str = "minibar.db", object_name: str = None) -> bool:
        """Выгружает файл базы в R2.

        db_path — локальный файл (может быть временным снимком),
        object_name — имя в бакете; по умолчанию основной minibar.db.
        """
        return await asyncio.to_thread(self._upload_file, db_path, object_name or self.DB_OBJECT)
